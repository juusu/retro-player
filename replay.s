...
.loopStart:
        move.l      a0,ch_DataStart(a1)              ;store beginning of ch0 pattern data
        move.l      a0,ch_DataRestart(a1)            ;init resetart ptr to the beginning of pattern data as well
        move.l      a0,ch_DataPtr(a1)                ;also init the current note pointer to the same place
...
.innerLoop:
        ...
        add.l       d0,ch_DataRestart(a1)            ;add offset to beginning of data

        cmp.w       #$ffff,(a0)+                                ;check if this was the last channel ?
        beq.s       .noteLoopEnd                                ;yeah, exit loop!

        lea         ch_sizeof(a1),a1                        ;no? next channel structure
...
.bufferLoop:
        ...
        move.l      d0,ch_BufferStart(a1)            ;store buffer start
        move.l      d0,ch_BufferWritePtr(a1)         ;store buffer write pointer

        asl         #2,d1                                       ;length is in longwords - convert to bytes
        add.l       d1,d0                                       ;keep track of total amount of mem needed for buffer in d0

        move.l      d0,ch_BufferEnd(a1)              ;store buffer end

        lea         ch_sizeof(a1),a1                        ;next channel structure
        dbf         d2,.bufferLoop
...
.updatePtrs:
        ...
        add.l       d0,ch_BufferStart(a1)
        add.l       d0,ch_BufferWritePtr(a1)
        add.l       d0,ch_BufferEnd(a1) 
        lea         ch_sizeof(a1),a1                        ;next channel structure
        dbf         d2,.updatePtrs
        bra.s       .compress
...
.getNextNote:
        tst.w       ch_ReadLength(a2)                ;do we have bytes to read from the buffer still?
        bne         .lookBack                                   ;yes, do it!

        move.l      ch_DataPtr(a2),a6                ;get current note pointer
.readNote:
        move.l      (a6)+,d1                                    ;read current note into D1
        move.l      a6,ch_DataPtr(a2)                ;store current channel note pointer
...
.processVolume
        ...
        move.w      d0,ch_VOL(a2)                    ;store the volume
...
.newNote:
        ...
        move.l      i_Sample_Start(a3,d0),a5
        adda.l      a4,a5
        move.l      a5,ch_PTR(a2)
        move.l      i_Loop_Start(a3,d0),a5
        adda.l      a4,a5        
        move.l      a5,ch_PTR_loop(a2)
        
        move.w      i_Sample_Length(a3,d0),ch_LEN(a2)
        move.w      i_Loop_Length(a3,d0),ch_LEN_loop(a2)
...
.noNewNote:
        ...
        move.l      ch_PTR_loop(a2),d3             ;get old pointer value
        add.l       d0,d3                                       ;add offset
        move.l      d3,ch_PTR_loop(a2)             ;store it back for later
...
.noPointerChange2:
        ...
        move.w      ch_PER(a2),d3                  ;get old period
        add.w       d1,d3                                       ;add offset
        move.w      d3,ch_PER(a2)                  ;store it back for later

.noPeriodChange:
.nextChannel:
        lea         ch_sizeof(a2),a2                        ;next channel structure
        dbf         d2,.loop
...
.writeBack
        ...
        move.l      ch_BufferWritePtr(a2),a5         ;read the buffer write pointer
        cmpa.l      ch_BufferEnd(a2),a5              ;check if we need to wrap around
        bne.s       .noWrap2

        move.l      ch_BufferStart(a2),a5            ;wrap back around to the beginning of the buffer
.noWrap2:
        move.l      d1,(a5)+                                    ;store the current note into the buffer
        move.l      a5,ch_BufferWritePtr(a2)         ;store the buffer write pointer
.exit:
        rts
...
.compression:
        ...
        move.w      d1,ch_ReadLength(a2)           ;store it
        asl.l       #1,d0
        and.l       #$7fff0000,d0
        ...
        move.l      ch_BufferWritePtr(a2),d1       ;get end of buffer
        sub.l       d0,d1

        cmp.l       ch_BufferStart(a2),d1          ;check for wrap
        blt.s       .wrapBuffer

        move.l      d1,ch_BufferReadPtr(a2)        ;store new read ptr
        bra         .getNextNote

.channelEnd:
        ;rewind channel data to the restart pointer
        move.l      ch_DataRestart(a2),a6
        bra         .readNote

.lookBack:
        move.l      ch_BufferReadPtr(a2),a5          ;current buffer read ptr
        cmpa.l      ch_BufferEnd(a2),a5              ;check if we need to wrap around
        bne.s       .noWrap       
...
.noWrap:
        move.l      (a5)+,d1
        subq.w      #1,ch_ReadLength(a2)
        move.l      a5,ch_BufferReadPtr(a2)
        bra         .processNote

.wrapBuffer:
        sub.l       ch_BufferStart(a2),d1
        add.l       ch_BufferEnd(a2),d1
...
.pokePaula:
        ...
        btst        d0,d6				
        beq.s       .noPokePtrs
        move.l      ch_PTR(a2),ac_ptr(a0,d1)         ;poke ac_ptr
        move.w      ch_LEN(a2),ac_len(a0,d1)         ;poke ac_len
.noPokePtrs:
        move.w      ch_PER(a2),ac_per(a0,d1)         ;poke ac_per
        move.w      ch_VOL(a2),ac_vol(a0,d1)         ;poke ac_vol
...
.rePokePaula:
        ...
        move.l      ch_PTR_loop(a2),ac_ptr(a0,d1)    ;always repoke lthe pointers so we can do wavetable stuff
                                                                ;replay code makes sure this doesn't change if not needes, so we can safely do this
 
        move.w      ch_LEN_loop(a2),ac_len(a0,d1)    ;poke ac_len
        lea         ch_sizeof(a2),a2                        ;next channel structure
...
;CIA player constants - only use if opt_CIA is set to non-zero
        ifne        opt_CIA
...
rc_DataPtr:
        ds.l        1
rc_CodePtr:
        ds.l        1                                           ;poke playroutine
;
; uninitialized vars from here on, feel free to use databss when assembling for kick2.0+ to save some bytes of .exe size
;
rc_IntSwitch:
        ds.w        1
rc_CIAResource:	
...
rc_SyncBytes:
        ...
        endc
;
;.define mod instrument structure
;
        rsreset
i_Sample_Start          rs.l    1
i_Loop_Start            rs.l    1
i_Sample_Length         rs.w    1
i_Loop_Length           rs.w    1
i_sizeof                rs.b    0
...
;
        rsreset
ch_DataStart            rs.l        1
ch_DataRestart          rs.l        1
ch_DataPtr              rs.l        1

; decompression pointers
ch_BufferStart          rs.l        1
ch_BufferEnd            rs.l        1
ch_BufferReadPtr        rs.l        1
ch_BufferWritePtr       rs.l        1
ch_ReadLength           rs.w        1
